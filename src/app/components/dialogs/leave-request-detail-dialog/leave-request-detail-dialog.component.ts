...
export class LeaveRequestDetailDialogComponent {
  public dialogRef: MatDialogRef<LeaveRequestDetailDialogComponent> = inject(MatDialogRef<LeaveRequestDetailDialogComponent>)
  ...
  rejectLeave() {
    ...
    this.dialogService.openDialogConfirm('Do you reject the leave request?').subscribe(result => {
      if (result) {
        this.approvalService.deleteLeaveRequest(this.data).subscribe({
          next: (data: any) => {
